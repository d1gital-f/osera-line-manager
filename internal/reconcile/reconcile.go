...
type Config struct {
	...
	// MavenWorkers is how many Maven runs go at once, four when zero.
	MavenWorkers int
	// MavenBatchSize is how many components one Maven run resolves, forty when zero.
	MavenBatchSize int
...
func New(ctx context.Context, cfg Config) (*Reconciler, error) {
	...
	if r.grype != nil {
		r.grype.Logf = highf
		r.grype.Say = func(format string, a ...any) { logf("    "+format, a...) }
	}
