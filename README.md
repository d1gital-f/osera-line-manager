...
2. For every line whose graph is missing or was built from another anchor, the anchor is resolved inside the pod with Maven, breadth first over everything the anchor manages, and written as CycloneDX under `graphs/<line_id>/maven.cdx.json`.
