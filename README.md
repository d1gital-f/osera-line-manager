...
2. For every line whose graph is missing, was built from another anchor or another roots rule, or carries unresolved nodes, the line is resolved inside the pod with Maven and written as CycloneDX under `graphs/<line_id>/maven.cdx.json`. The roots are the line's projects, not everything the anchor manages: the anchor's managed artifacts whose group is one of the declared `components`' groups (the anchor's own group counts), plus a declared component the anchor does not manage, at its version; the anchor still supplies every version. A line with no components falls back to every managed artifact, the wide graph. The rule is recorded in the file (`osera:roots`). The probes run in batches: one Maven run resolves `MAVEN_BATCH_SIZE` components (forty by default) as the modules of a throwaway multi module project, `MAVEN_WORKERS` runs at a time; a module whose tree is missing is probed alone, so nothing is lost.
