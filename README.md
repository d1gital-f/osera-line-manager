...
The server answers `GET /healthz`, `GET /status/<line_id>.json` from the clone, `POST /webhook` for Nexus, every delivery checked against the secret, and `POST /pass` to run a pass by hand (`kubectl -n line-manager port-forward deploy/line-manager 8080 && curl -X POST localhost:8080/pass`). An accepted event on the release repository, or a request on `/pass`, starts one more pass as soon as the running one ends.
...
- `high`: `medium` plus the detail: every Maven run with its duration, every fallback, every graph kept, every staged file, the Grype database status, the dev advisory file, `validate` pending.
...
2. For every line whose graph is missing, was built from another anchor, another roots rule or other declared components, or carries unresolved nodes, the line is resolved inside the pod with Maven and written as CycloneDX under `graphs/<line_id>/maven.cdx.json`. The line is built the way a bank builds it: one throwaway project whose dependencies are the roots, with the anchor imported, one Maven run, and the resolved tree is the graph, one version of every library because Maven has mediated. The roots are the line's projects, not everything the anchor manages: the anchor's managed artifacts (the BOMs it imports followed) whose group is one of the declared `components`, the anchor's own group included. A component is written `group@version`, a group of the line's projects at the version the line wants: the group is the scope, the version pins the whole group when the anchor manages it at another. Wave 1 writes org.springframework@5.3.39 on the Boot row, Boot 2.7.18 manages 5.3.31, so every Spring Framework artifact moves to 5.3.39, as explicit entries before the import so they win. A component in any other form is an error and the line is skipped, never built wrong. A declared group the anchor manages nothing of gives no roots and the log says so. A line with no components takes every artifact the anchor lists, which for a project's own BOM (spring-framework-bom, spring-security-bom) is exactly its modules. The rule, the components and the pins are recorded in the file (`osera:roots`, `osera:components`, `osera:pins`, `osera:method`). If the build fails, the older method takes over and the file says so: every root probed alone in batches (`MAVEN_BATCH_SIZE` components per Maven run, `MAVEN_WORKERS` runs at a time), breadth first until nothing new appears. For a list like Boot's with no components that is the wide graph.
3. For every line whose graph is new, whose backlog is empty, whose last scan is older than the rescan interval, or whose rules file or advisory file changed since its last scan (the scan record carries their digest): the graph's components go to OSV, the CISA KEV list, FIRST EPSS and NVD, through disk caches, the rules file says which findings are applicable, and the result is merged into `cve-backlog.json` (an entry already there keeps its status and takes the new facts, a new one is added open, one the scan no longer finds stays), `cve-excluded.json` carries the rest with a reason, `cve-backlog.md` is the table.
4. The board is read once, one paged query, the lanes included. The release repository is listed once; the evidence file next to every patched coordinate is read and cached once found, and read again every pass while it is missing, since it can land after the jar.
5. One record per line: an entry is fixed while a promoted coordinate on its library and version names its CVE, the highest patch wins; not remediable from the label on its issue; in progress when its issue sits in a patch repository (an issue is one per CVE and library version, the unit a producer patches, read from its title "CVE-x in <artifact> <version>"; an older issue titled by the CVE alone stands for every version), one named `patch-<project>`, an issue moved anywhere else leaves the entry open and is listed as a discrepancy; open otherwise. The counts add up to the entries in scope. Discrepancies are listed for a person. The entries go back into the backlog file with their status.
...
9. The issue of every entry that became fixed or not remediable this pass is closed with the coordinate and the BOM, or the producer's declaration; an issue still open for an entry fixed earlier is closed too. An issue whose label was withdrawn is reopened. The card of every claimed issue (in a patch repository, open) is put in the board's In Progress lane, the one board write: a transfer moves the issue, not its card.
10. One sentence per line: the word, the counts, what banks consume.
